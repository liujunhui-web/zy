...
http.interceptors.response.use(
  response => {
    ...
    //   //       name: 'login',
    //   //       query: {
    //   //         from: window.this.$route.name,
    //   //         params: window.this.$route.query
    //   //       }
    //   //     })
    //   //     break
    //   //   default:
    ...
    //   session.setSession('sessionid', response ?.headers ?.sessionid)
    //   return response
    // } else {
    //   return res.data
    // }
    ...
    return new Promise((resolve, reject) => {
      http
        .get(url, {
          ...
          paramsSerializer: x => Qs.stringify(x, {
            arrayFormat: 'repeat'
          })
        })
        .then(resolve)
...
export const form = url => {
  return (data = {}) => {
    return new Promise((resolve, reject) => {
      ...
      http
        .post(url, Qs.stringify(JSONClone(data), {
          arrayFormat: 'repeat'
        }))
        .then(resolve)
        .catch(reject)
...
export const binary = url => {
  return (objOrFile, cbProgress) => {
    ...
    if (Object.prototype.toString.call(objOrFile) === '[object Object]') {
      // 是对象 就序列化
      ...
      http
        .post(url, postForm, {
          ...
          onUploadProgress: ({
            ...
            // 进度条实现
            cbProgress({
              percent: Math.round((loaded / total) * 100) - 10
            }) // 上传完成为90
          }
        })
        .then(res => {
...
export const download = url => {
  ...
  return (params = {}, name) => {
    ...
    axios({
      ...
      responseType: 'blob',
      headers: {
        // sessionid: session.getSession('sessionid').replace(/"/g, '')
      }
    }).then(res => {
      jsFileDownload(res.data, params.name)
    })
  }
}
