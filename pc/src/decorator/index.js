...
import { cantUse } from '@/utils'
import { WarningMessage } from '@/utils/antdvUtils'
 ...
 */
export function hasPermission (value, message = '您没有当前操作的权限') {
  return function (target, name, descriptor) {
    if (!value) throw new Error(`没有传入权限函数${name}`)
    if (cantUse(value)) {
      descriptor.value = () => WarningMessage(`${message}--权限id:(${value})`)
    } else {
      globalLog(`生成函数${name}准备运行,权限%c${value}`, consoleFormat)
      return descriptor
    }
  }
}
